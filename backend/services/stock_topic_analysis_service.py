...
def _call_stock_analysis_ai(prompt_payload: str, *, incremental: bool = False) -> Tuple[str, str]:
    ...
    user_prompt = (
        "请基于输入摘要片段，对这只股票生成中文 Markdown 公司调研报告。\n\n"
        "结构：\n"
        "## 公司调研报告\n"
        "### 1. 投资摘要\n"
        "用 3-5 条概括当前最重要的结论，突出最值得关注的变化和判断。\n\n"
        "### 2. 公司定位与核心业务\n"
        "总结公司所处产业链位置、主要产品/服务、客户或下游方向。只使用输入摘要片段中的信息。\n\n"
        "### 3. 核心逻辑与成长驱动\n"
        "总结卖方/调研材料中反复出现的主线，例如需求增长、国产替代、技术升级、产能释放、订单变化、价格变化等。\n\n"
        "### 4. 关键数据、预测与口径\n"
        "整理摘要片段中出现的收入、利润、订单、产能、出货量、目标价、市值空间等数字及其口径；没有明确数值时写未提及。\n\n"
        "### 5. 催化事件\n"
        ...
        "- 只基于输入摘要片段，不要补充外部行情、财报或未出现的信息。\n"
        "- 输入的 new_topics.excerpt 是针对当前股票从原话题抽取的摘要片段；全文都讲该股票时可能是全文，多股票分段时通常只保留当前股票相关段落。\n"
        "- 重点提炼可用于后续跟踪的结论、驱动因素、数据变化和事件进展。\n"
        "- 如果输入中出现数字或预测值，请在“关键数据、预测与口径”中整理；没有明确数值时直接写未提及。\n"
        "- 如果某章节缺少摘要片段支撑，直接写未提及或保持简短，不要为了完整而扩写。\n"
        "- existing_summary_markdown 可能来自历史保存报告，也可能来自本次初始化分析的上一批结果；如果输入中包含它，请沿用原有章节结构，只在对应章节内补充新增话题，不要只输出差异或单独列新章节。\n"
        "- 增量分析时，先判断新增摘要片段是否提供实质新增信息；如果没有，只做必要的小幅修正或保持原章节内容，不要为了增量而强行扩写。\n"
        "- 如果输入中出现新的观点或更新，只把它合并进最相关的既有章节，不要新增流水账段落。\n"
        "- 如果旧报告里已经写过相同或相近意思的内容，不要重复添加，优先去重和保留原有表述。\n"
        "- 结论只总结摘要片段中的研究判断和跟踪变量，不输出未由材料支持的投资评级或操作建议。\n"
        f"输入数据：\n{prompt_payload}"
    )
    messages = [
        {
            ...
            "content": (
                "你是A股卖方调研材料分析助手。"
                "只基于用户提供的知识星球摘要片段生成专业公司调研报告，不要补充外部行情或未出现的信息。"
            ),
        },
        {
            "role": "user",
