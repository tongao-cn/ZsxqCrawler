...
def _call_stock_analysis_ai(prompt_payload: str, *, incremental: bool = False) -> Tuple[str, str]:
    ...
    user_prompt = (
        "请基于输入证据片段，对这只股票生成中文 Markdown 公司调研报告。\n\n"
        "结构：\n"
        "## 公司调研报告\n"
        "### 1. 投资摘要\n"
        "用 3-5 条概括当前最重要的结论，突出最值得关注的变化和判断。\n\n"
        "### 2. 公司定位与核心业务\n"
        "总结公司所处产业链位置、主要产品/服务、客户或下游方向。只使用输入证据片段中的信息。\n\n"
        "### 3. 核心逻辑与成长驱动\n"
        ...
        "- 如果新增证据片段没有提供实质新增信息，只做必要的小幅修正或保持原章节内容，不要为了增量而强行扩写。\n"
