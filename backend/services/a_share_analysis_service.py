...
def _coverage_layer(rank_30: Optional[int]) -> Tuple[str, str, int]:
    if rank_30 is None:
...
def _build_coverage_pool(
    ...
    for window, top_n in (
        ...
        *DEFAULT_COVERAGE_SHORT_WINDOWS,
        ...
        rows_by_window[window] = _build_ranking_rows(daily, available_dates, from_index, end_index, top_n)
    ...
    if previous_end_index >= start_index:
        ...
        previous_primary_rows = _build_ranking_rows(
            daily,
            available_dates,
            previous_from_index,
            previous_end_index,
    ...
    primary_rows = _attach_rank_movement(rows_by_window[DEFAULT_COVERAGE_PRIMARY_WINDOW], previous_primary_rows)
    items_by_company: Dict[str, Dict[str, Any]] = {}

    for row in primary_rows:
        company = str(row.get("company") or "")
        rank_30 = int(row.get("rank") or 0)
        layer, layer_label, layer_order = _coverage_layer(rank_30)
    ...
    for item in items_by_company.values():
        ...
        layer, layer_label, layer_order = _coverage_layer(rank_30 if isinstance(rank_30, int) else None)
