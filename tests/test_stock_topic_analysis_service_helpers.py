...
class StockTopicAnalysisServiceHelperTests(unittest.TestCase):
    ...
    def test_stock_topic_analysis_engine_delegates_typed_requests(self):
        ...
        engine = StockTopicAnalysisEngine(operations)
        log_callback = lambda message: None

        self.assertEqual(
            {"kind": "one"},
            engine.analyze_stock_topics("group-1", "宁德时代", limit=3, log_callback=log_callback),
        )
        self.assertEqual(
            {"kind": "batch"},
            engine.analyze_stock_topics_batch("group-1", ["宁德时代"], log_callback=log_callback, max_stocks=1),
        )
        self.assertEqual(
            {"kind": "question"},
            engine.answer_stock_question("group-1", "固态电池怎么看", log_callback=log_callback),
        )
    ...
    @unittest.skipUnless(HAS_SERVICE_DEPS, "stock topic analysis service dependencies are not installed")
    def test_stock_topic_service_wrappers_delegate_to_engine(self):
        from backend.services import stock_topic_analysis_service as service

        log_callback = lambda message: None

        with patch.object(service._STOCK_TOPIC_ANALYSIS_ENGINE, "analyze_stock_topics", return_value={"kind": "one"}) as run_one:
            self.assertEqual(
                {"kind": "one"},
                service.analyze_stock_topics("group-1", "宁德时代", limit=2, log_callback=log_callback),
            )
        run_one.assert_called_once_with("group-1", "宁德时代", limit=2, log_callback=log_callback)

        with patch.object(
            service._STOCK_TOPIC_ANALYSIS_ENGINE,
            "analyze_stock_topics_batch",
            return_value={"kind": "batch"},
        ) as run_batch:
            self.assertEqual(
                {"kind": "batch"},
                service.analyze_stock_topics_batch("group-1", ["宁德时代"], log_callback=log_callback, max_stocks=1),
            )
        run_batch.assert_called_once_with("group-1", ["宁德时代"], log_callback=log_callback, max_stocks=1)

        with patch.object(
            service._STOCK_TOPIC_ANALYSIS_ENGINE,
            "answer_stock_question",
            return_value={"kind": "question"},
        ) as run_question:
            self.assertEqual(
                {"kind": "question"},
                service.answer_stock_question("group-1", "固态电池怎么看", log_callback=log_callback),
            )
        run_question.assert_called_once_with("group-1", "固态电池怎么看", log_callback=log_callback)
    ...
    def test_extract_stock_names_from_image_parses_ai_json(self):
        from backend.services.ai_runtime_request import AIRuntimeStructuredObjectResult
        from backend.services.stock_topic_image_extraction import extract_stock_names_from_image
        ...
        encoded = base64.b64encode(b"image-bytes").decode("ascii")
        with patch("backend.services.stock_topic_image_extraction.call_structured_ai_object", side_effect=fake_call):
    ...
    def test_extract_stock_names_from_image_rejects_empty_result(self):
        ...
        encoded = base64.b64encode(b"image-bytes").decode("ascii")
        with patch(
            "backend.services.stock_topic_image_extraction.call_structured_ai_object",
