...
class PostgresCoreSchemaTests(unittest.TestCase):
    def test_core_schema_sql_contains_schema_tables_and_indexes(self):
        ...
        self.assertIn(f'CREATE TABLE IF NOT EXISTS "{CORE_SCHEMA}"."stock_topic_analyses"', sql)
        self.assertIn(f'CREATE TABLE IF NOT EXISTS "{CORE_SCHEMA}"."stock_topic_analysis_versions"', sql)
        self.assertIn(f'CREATE TABLE IF NOT EXISTS "{CORE_SCHEMA}"."stock_topic_processed_states"', sql)
        self.assertIn(f'CREATE TABLE IF NOT EXISTS "{CORE_SCHEMA}"."task_locks"', sql)
        self.assertIn('PRIMARY KEY ("lock_key")', sql)
        self.assertIn('CREATE UNIQUE INDEX IF NOT EXISTS "daily_stock_concepts_group_id_report_date_stock_name_key"', sql)
        self.assertIn('CREATE UNIQUE INDEX IF NOT EXISTS "stock_topic_analyses_group_id_stock_name_key"', sql)
        self.assertIn('CREATE UNIQUE INDEX IF NOT EXISTS "stock_topic_processed_states_group_id_stock_name_topic_id_ke"', sql)
        ...
        self.assertIn('CREATE INDEX IF NOT EXISTS "idx_task_locks_group_id_category_expires_at"', sql)
        self.assertIn('CREATE INDEX IF NOT EXISTS "idx_stock_topic_processed_states_group_id_stock_name_status"', sql)
        self.assertIn('CREATE INDEX IF NOT EXISTS "idx_stock_topic_analysis_versions_group_id_stock_name_analys"', sql)
