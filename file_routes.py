...
from file_ai_analysis_service import (
...
def run_collect_files_task(task_id: str, group_id: str, request: FileCollectRequest):
    ...
    try:
        ...
        file_downloader_instances = _get_main_attr("file_downloader_instances")
        get_cookie_for_group = _get_main_attr("get_cookie_for_group")
...
def run_single_file_download_task_with_info(
    ...
    try:
        ...
        downloader = ZSXQFileDownloader(cookie=cookie, group_id=group_id, db_path=db_path)
        downloader.log_callback = log_callback
        downloader.stop_check_func = stop_check
        file_downloader_instances[task_id] = downloader
...
async def collect_files(group_id: str, request: FileCollectRequest, background_tasks: BackgroundTasks):
    """收集文件列表"""
    try:
        create_task = _get_main_attr("create_task")
...
async def get_file_status(group_id: str, file_id: int):
    ...
    try:
        ...
        safe_filename = _safe_filename(file_name, f"file_{file_id}")
...
async def check_local_file_status(group_id: str, file_name: str, file_size: int):
    ...
    try:
        path_manager = get_db_path_manager()
        safe_filename = _safe_filename(file_name, file_name)
        download_dir = os.path.join(path_manager.get_group_dir(group_id), "downloads")
        file_path = os.path.join(download_dir, safe_filename)
