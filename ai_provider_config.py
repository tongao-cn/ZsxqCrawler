"""Resolve OpenAI-compatible model settings from env vars or Codex config."""
...
DEFAULT_CODEX_CONFIG_PATH = Path(os.getenv("CODEX_CONFIG_PATH", r"C:\Users\g1121\.codex\config.toml"))
DEFAULT_CODEX_AUTH_PATH = Path(os.getenv("CODEX_AUTH_PATH", r"C:\Users\g1121\.codex\auth.json"))
...
def get_openai_compatible_config() -> Dict[str, Any]:
    ...
    codex_config = _load_toml_file(DEFAULT_CODEX_CONFIG_PATH)
    provider_name = str(codex_config.get("model_provider") or "OpenAI").strip() or "OpenAI"
    provider_config = ((codex_config.get("model_providers") or {}).get(provider_name) or {})
    codex_auth = _load_json_file(DEFAULT_CODEX_AUTH_PATH)

    model = env_model or str(codex_config.get("model") or DEFAULT_FALLBACK_MODEL).strip() or DEFAULT_FALLBACK_MODEL
    base_url = env_base_url or str(provider_config.get("base_url") or DEFAULT_FALLBACK_BASE_URL).strip() or DEFAULT_FALLBACK_BASE_URL
    wire_api = env_wire_api or str(provider_config.get("wire_api") or DEFAULT_FALLBACK_WIRE_API).strip() or DEFAULT_FALLBACK_WIRE_API
    reasoning_effort = env_reasoning_effort or DEFAULT_FALLBACK_REASONING_EFFORT
    api_key = env_api_key or str(codex_auth.get("OPENAI_API_KEY") or "").strip()
    ...
    return {
        ...
        "codex_config_path": str(DEFAULT_CODEX_CONFIG_PATH),
        "codex_auth_path": str(DEFAULT_CODEX_AUTH_PATH),
    }
