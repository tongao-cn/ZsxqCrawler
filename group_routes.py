...
async def refresh_local_groups():
    ...
    """
    try:
        ids = await asyncio.to_thread(_get_main_attr("scan_local_groups"))
        return {"success": True, "count": len(ids), "groups": sorted(list(ids))}
    except Exception as e:
        cached = _get_main_attr("get_cached_local_group_ids")(force_refresh=False) or set()
        # 不报错，返回降级结果
...
async def get_groups():
    ...
    try:
        # 自动构建群组→账号映射（多账号支持）
        group_account_map = _get_main_attr("build_account_group_detection")()
        local_ids = _get_main_attr("get_cached_local_group_ids")(force_refresh=False)

        # 获取“当前账号”的群列表（优先账号默认账号，其次config.toml；若未配置则视为空集合）
        groups_data: List[dict] = []
        try:
            primary_cookie = _get_main_attr("get_primary_cookie")()
            if primary_cookie:
                groups_data = _get_main_attr("fetch_groups_from_api")(primary_cookie)
        except Exception as e:
            # 不阻断，记录告警
    ...
    try:
        ...
        def build_fallback(source: str = "fallback", note: str = None) -> dict:
            ...
            result = {
                ...
                "background_url": None,
                "account": _get_main_attr("get_account_summary_for_group_auto")(group_id),
                "source": source,
            }
            if note:
                result["note"] = note
...
async def delete_group_local(group_id: str):
    """删除指定社群的本地数据（复用主模块实现）"""
    return await _get_main_attr("delete_group_local")(group_id)
